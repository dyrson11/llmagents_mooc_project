...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--sys",
        type=str,
        help="File with each line-by-line model outputs",
    )
    parser.add_argument(
        "--gold_reference",
        "--ref",
        type=str,
        help="File with corresponding line-by-line references",
    )
    parser.add_argument(
        "--detailed_output",
        action="store_const",
        const=True,
        default=False,
        help="(sacrebleu) Print additional BLEU information (default=False)",
    )
    ...
    calculate_score_report(
        system_lines, [gold_lines], score_only=not args.detailed_output
    )
